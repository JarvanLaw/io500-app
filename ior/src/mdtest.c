...
static void create_remove_dirs (const char *path, bool create, uint64_t itemNum) {
    ...
    //printf("path: %s, name: dir.%s%\n" PRIu64, path, create ? mk_name : rm_name, itemNum);
    //printf("create_remove_items_helper (dirs %s): curr_item is '%s'\n", operation, curr_item);
...
static void create_file (const char *path, uint64_t itemNum) {
    ...
    //printf("create_remove_items_helper (non-dirs create): curr_item is '%s'\n", curr_item);
...
void create_remove_items(int currDepth, const int dirs, const int create, const int collective, const char *path, uint64_t dirNum, rank_progress_t * progress) {
    ...
    //printf("create_remove_items (start): temp_path is '%s'\n", temp_path );
    ...
    if (currDepth == 0) {
        ...
        for (i=0; i<branch_factor; i++) {
            ...
            //printf("create_remove_items (for loop): temp_path is '%s'\n", temp_path );
...
void collective_create_remove(const int create, const int dirs, const int ntasks, const char *path, rank_progress_t * progress) {
    ...
    printf("Entering collective_create_remove on %s\n", path );
...
void display_freespace(char *testdirpath)
{
    ...
    VERBOSE(3,5,"Before or WARNING: unable to use realpath() on file systemshow_file_system_size, dirpath is '%s'", dirpath );
...
void create_remove_directory_tree(int create,
	...
	//printf("Entering create_remove_directory_tree on %s, currDepth = %d...", path, currDepth);
    if (currDepth == 0) {
...
static void mdtest_iteration(int i, int j, MPI_Group testgroup, mdtest_results_t * summary_table){
  ...
  for (int dir_iter = 0; dir_iter < directory_loops; dir_iter ++){
	...
	//printf("rank: %d, path_count: %d, iteration %d, access testdir: %s\n", rank, path_count, j+1, testdir);
